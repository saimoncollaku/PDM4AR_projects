...
def se2transform_to_string(config: SE2Transform):
    return f"({config.p[0]:.1f}, {config.p[1]:.1f}; deg:{np.rad2deg(config.theta): .1f})"
...
class Line(Segment):
    ...
    def __str__(self) -> str:
        return f"S{'-' if self.gear is Gear.REVERSE else ''}(len - {self.length :.1f}) - ({se2transform_to_string(self.start_config)} to {se2transform_to_string(self.end_config)})"
...
class Curve(Segment):
    ...
    def __str__(self) -> str:
        st = f"{'-' if self.gear is Gear.REVERSE else ''}(len - {self._arc_angle * self.radius:.1f}, ang - {np.rad2deg(self._arc_angle):.2f}) - ({se2transform_to_string(self.start_config)} to {se2transform_to_string(self.end_config)}) - (cen: {se2transform_to_string(self.center)})"
        return "L" + st if self.type is DubinsSegmentType.LEFT else "R" + st
